...
void setup() {
  ...
  //Makes sure the cursor size slider is properly adjusted
  cursor_rslidermap();
...
void draw() {
  ...
  if (mode == start) {
    ...
    cursorsettings();
  } else if (mode == shop_settings) {
    shop();
  } else if (mode == leaderboard) {
    leaderboard();
  } else if (mode == target_settings) {
    targetsettings();
  } else if (mode == achievements_settings) {
  ...
  if (keyCode == ESC) {    
    ...
    if (cursorspin == false && game_mode != playing && game_mode != paused && game_mode != game_over) {
      ...
      if (mode == settings || mode == game || mode == leaderboard) {
        ...
        mode = settings;
      } else if (mode == shop_settings || mode == achievements_settings) {
