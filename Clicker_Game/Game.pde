...
//Saves target skin options
void targetsavepref(boolean skinunlock, int skintochange) {

  String[] tpreferences1 = loadStrings("targetsave.txt");
  
  int[] tpreferences2 = new int[10];
  tpreferences2[0] = targetskin;
  tpreferences2[1] = Math.round(targetD);
  tpreferences2[2] = Integer.parseInt(tpreferences1[2]);
  tpreferences2[3] = Integer.parseInt(tpreferences1[3]);
  tpreferences2[4] = Integer.parseInt(tpreferences1[4]);
  tpreferences2[5] = Integer.parseInt(tpreferences1[5]);
  ...
  String[] preferencesstring = str(tpreferences2);
  
  //Saves it to file
  saveStrings("targetsave.txt", preferencesstring);
}
...
void timer(float x, float y) {
  if (secondsleft > 0 && game_mode == playing) {
    secondsleft = currentsecond - (second() + minute() * 60) + gametime * timebonus + pausedtimet;
  }
...
void scoreincrease() {
  if (dist(mouseX, mouseY, targetX, targetY) <= targetD/2 && game_mode == playing) {
    ...
    scorecountersize();
    scoreup = false;
  } else if (score > 0 && game_mode == playing && lavabonus == 1) {
    score--;
    scoresize--;
  }
}
...
void gameend() {
  if (secondsleft == 0) {
    ...
    newbalance = (goldbonus)*(Math.round(score/2) + 20);
    if (balance + newbalance <= 999) {
      balance = balance + newbalance;
    } else {
      newbalance = 999 - balance;
      balance = 999;
    }
